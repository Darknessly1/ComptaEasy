...
const NewsCard = ({ title, description, imageUrl, sourceUrl }) => (
  <div className=" overflow-hidden bg-gray-200/60 p-6 rounded-md shadow-md">
    {imageUrl && <img className="mb-4 rounded-md w-full h-32 object-cover no-blur" src={imageUrl} alt={title} />}
...
const HomePage = () => {
  ...
  const filteredNews = news.filter((newsItem) => newsItem.urlToImage);
